...
module.exports = (name, save, schema, options) => {
  ...
  const ignoreTagForSubSchema =
    options && options.ignoreTagForSubSchema
      ? options.ignoreTagForSubSchema
      : false
  ...
  if (!properties[save.idProperty]) {
    throw new Error(
      `schema does not have the required property '${save.idProperty}'`
    )
  }
  ...
  return Object.assign(self, {
    ...
    create(object, createOptions, callback) {
      ...
      const cleanObject = schema.cast(
        schema.stripUnknownProperties(
          ...
          createOptions.persist || createOptions.tag,
      ...
      pipe(
        ...
        (error, pipedObject) => {
          ...
          schema.validate(
            pipedObject,
            createOptions.set,
            createOptions.validate || createOptions.tag,
    ...
    read(id, callback) {
      return save.read(
        schemata.castProperty(properties[save.idProperty].type, id),
        (error, object) => {
          if (error) return callback(error)
    ...
    update(object, updateOptions, callback) {
      ...
      const cleanObject = schema.cast(
        schema.stripUnknownProperties(
          schema.makeDefault(object),
          updateOptions.persist || updateOptions.tag,
          ignoreTagForSubSchema
        )
      )
      ...
      pipe(
        ...
        (error, pipedObject) => {
          ...
          schema.validate(
            pipedObject,
            updateOptions.set,
            updateOptions.validate || updateOptions.tag,
            (error, validationErrors) => {
              if (error) return callback(error)
              if (Object.keys(validationErrors).length > 0) {
                const validationError = new Error('Validation Error')
    ...
    partialUpdate(object, updateOptions, callback) {
      ...
      if (!object[save.idProperty]) {
        return callback(
          new Error(`object have not ID property '${save.idProperty}'`)
        )
      }
      ...
      save.read(object[save.idProperty], (error, readObject) => {
        ...
        if (typeof readObject === 'undefined') {
          return callback(
            new Error(
              `Couldn't find object with an ${save.idProperty} of ${
                object[save.idProperty]
              }`
            )
          )
        }
                  ...
                  (error, partialUpdatePipedObject) => {
                    ...
                    save.update(
                      ...
                      (error, savedObject) => {
                        ...
                        callback(
                          null,
                          schema.stripUnknownProperties(savedObject)
                        )
                      }
                    )
                  }
    ...
    find(query, options, callback) {
      ...
      save.find(query, options, (error, objects) => {
        ...
        callback(
          null,
          objects.map(object => schema.stripUnknownProperties(object))
        )
      })
    },
